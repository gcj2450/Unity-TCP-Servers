...
using NetworkServers.NetworkServers.Module.Game.Model;
...
namespace NetworkServers.NetworkServers.Module.Game.Net
{
    class GateNet : Singleton<GateNet>
    {
        public GateNet()
        {
        ...
        public void DRAWONECARD_REQ(object data, Socket client)
        {
            ...
            int index = random.Next(0, GateManager.allServers.Count);
            //生成数据模型
            Game_Dto game_Dto = new Game_Dto(client.RemoteEndPoint.ToString(), data);
            MessageCenter.instance.Send(OperateCode.DRAWONECARD_REQ, game_Dto, GateManager.allServers[index].Client);
        }
        ...
        private void DRAWONECARD_REP(object data, Socket client)
        {
            ...
            MessageCenter.instance.Send(OperateCode.DRAWONECARD_REP, game_Dto.data, GateManager.allClientsDic[game_Dto.ipProt].Client);
        }
    }
}
